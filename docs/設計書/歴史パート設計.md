...
> **注: スクリーニング基準はパイプラインに依存せず、スコープベースのIC/ECで判定する。** パイプライン（P1–P9）はスクリーニング後の分類タグとしてのみ使用する。歴史パートの「箱」= パイプライン段タグ = LLMパートの分類軸、として統一されているが、採否判定自体はパイプラインによらない。
>
> スクリーニングの採択条件（すべて満たす）:
...
>
> 歴史パートの古典文献もこの基準で採否を判定する。コンパイラ理論の古典（SSA等）はIC1を満たさないためコーパス外とし、歴史パート本文では「理論的基盤」として引用に留める。
>
> **検索期間に制限は設けない（全年代対象）**。1960年代の古典から最新研究まで、DB自動検索で同一に収集する。歴史パートの文献を引用追跡のみに頼る必要はなく、DB検索でも直接ヒットする設計とする。LLMパートの対象は年代ではなく手法タグ（M-neural / M-llm / M-hybrid / M-stat）で絞り込む。
> 詳細は全体設計 ステップ1.3、ステップ3.2を参照。
