...
def cluster_entries(entries: list[dict], threshold: float) -> list[list[int]]:
    ...
    normalized_titles = [normalize_title(entry.get("title", "")) for entry in entries]

    for i in range(len(entries)):
        for j in range(i + 1, len(entries)):
            similarity = title_similarity(normalized_titles[i], normalized_titles[j])
...
class DedupTitleHandler(StepHandler):
    ...
    def run(self, input_entries: list[dict], config: dict) -> StepResult:
        threshold = float(config.get("similarity_threshold", 0.92))
        ...
        for index, member_indices in enumerate(clusters, start=1):
            ...
            rep_norm = normalize_title(representative.get("title", ""))
            ...
            for idx in member_indices:
                ...
                entry_key = entry.get("ID", "unknown")
                similarity = title_similarity(rep_norm, normalize_title(entry.get("title", "")))
